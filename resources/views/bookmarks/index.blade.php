    ...
    <div x-show="formOpen" x-cloak x-transition class="mb-10 p-5 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
        <form method="POST" action="{{ route('bookmarks.store') }}" class="grid gap-3 sm:grid-cols-2">
            ...
            {{-- Tag chip input: typing a comma or pressing Enter turns the
                 current text into a chip immediately (or picks the
                 highlighted autocomplete suggestion if there is one);
                 backspace on an empty field removes the last chip. The
                 hidden input carries the comma-joined list under the same
                 "tags" name the controller already parses. --}}
            <div
                x-data="tagInput([], @js($availableTags))"
